Sandrocoin Core version 0.13.2 is now available from:

  <https://download.sandrocoin.org/sandrocoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/sandrocoin-project/sandrocoin/issues>
...
ECDSA signatures inside Sandrocoin transactions now use validation using
[libsecp256k1](https://github.com/sandrocoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/sandrocoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Sandrocoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Sandrocoin Core was to raise the default minimum
relay fee.

Sandrocoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Sandrocoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running sandrocoind without having to do any manual configuration.
...
Sandrocoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Sandrocoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Sandrocoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Sandrocoin Core 0.13, and it is at this time undecided whether the
...
Sandrocoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Sandrocoin Core automatically creates a hidden service to listen on, without
manual configuration. Sandrocoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Sandrocoin Core is listening, and
...
Sandrocoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Sandrocoin
Core to use floating fees. By default, Sandrocoin Core will use floating
fees.
...
(default: `0.0002` SND/kB).

At all times, Sandrocoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SND.
Furthermore, Sandrocoin Core will never create transactions paying less than
...
`sandrocoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Sandrocoin Core v0.11.0](https://github.com/sandrocoin/sandrocoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/sandrocoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/sandrocoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /sandrocoinrpc>
        ...
        # optional bypass sandrocoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< sandrocoinrpc:<password>
    </Location>
...
Restarting sandrocoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in sandrocoin.conf
...
sandrocoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/sandrocoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Sandrocoin Core code base has
...
- `sandrocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `sandrocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/sandrocoin/sandrocoin/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the Sandrocoin scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/sandrocoin/sandrocoin/pull/7524>
...
Sandrocoin transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/sandrocoin/sandrocoin/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/sandrocoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Sandrocoin Core.

[Pull request](https://github.com/sandrocoin/sandrocoin/pull/8035/files), [BIP 32](https://github.com/sandrocoin/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/sandrocoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/sandrocoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Sandrocoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/sandrocoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/sandrocoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/sandrocoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/sandrocoin/sandrocoin/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Sandrocoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Sandrocoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Sandrocoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of sandrocoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of sandrocoins being spent is exactly
...
- **Increased security for multisig:** Sandrocoin addresses (both P2PKH addresses
  ...
  believed to be provided by Sandrocoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Sandrocoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Sandrocoin's current ledger (called the UTXO set),
  ...
  note that Sandrocoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Sandrocoin users to individually opt-in to almost any change in the Sandrocoin
  Script language when those users receive new transactions.  Features
  currently being researched by Sandrocoin and Sandrocoin Core contributors that may
  ...
  Other Sandrocoin community members are studying several other improvements
...
segwit's start date of 1 January 2017 miners can update the Sandrocoin
client to Sandrocoin Core 0.13.2 to signal for segwit support. When a
...
[Segwit FAQ]: https://sandrocoincore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://sandrocoincore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/sandrocoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/sandrocoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/sandrocoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/sandrocoin/bips/blob/master/bip-0145.mediawiki
...
Since Sandrocoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/sandrocoin/bips/blob/master/bip-0147.mediawiki
...
- `sandrocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `sandrocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
