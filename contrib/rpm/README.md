...
The RPM spec file provided here is for Sandrocoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sandrocoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sandrocoin git repository](https://github.com/sandrocoin/sandrocoin) but are not part of
...
Currently only `Source10` is used, for the example `sandrocoin.conf` file.
...
in the sandrocoin git repository. At present only an SVG version of the Sandrocoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sandrocoin
...
needed when 0.12.1 is released, a proper fix is already in the Sandrocoin git
master branch.
...
idea to build Sandrocoin Core against a static version of that library compiled
...
The `sandrocoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sandrocoin-server` package. At this point, most RPM based
...
The `sandrocoin-server` package should have SELinux support. How to properly do
...
Sandrocoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sandrocoin/). He has been
packaging Sandrocoin for Fedora at least since 2012.
...
4. I renamed the `sandrocoin` package that contains the Qt GUI to `sandrocoin-core` as
that appears to be how the general population refers to it, in contrast to
`sandrocoin-xt` or `sandrocoin-classic`. I wanted to make sure the general population
